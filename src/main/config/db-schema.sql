...
CREATE TABLE `wp_lh_rpt_channel_stay_night` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  ...
  `booking_source_raw` varchar(100) NOT NULL DEFAULT '',
  `booking_source` varchar(100) DEFAULT NULL,
  `source_category` varchar(50) DEFAULT NULL,
  `room_rate` decimal(12,2) DEFAULT NULL,
  `room_revenue` decimal(12,2) NOT NULL DEFAULT '0.00',
  `rooms_sold` int(10) unsigned NOT NULL DEFAULT '1',
  `data_origin` varchar(20) NOT NULL,
  `fetched_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_channel_stay_night` (`stay_date`,`reservation_id`,`room_id`,`booking_source_raw`),
  KEY `idx_channel_stay_date` (`stay_date`),
  KEY `idx_channel_stay_source` (`booking_source`,`stay_date`),
  KEY `idx_channel_stay_res` (`reservation_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Production migration: src/main/config/migrations/2026-09-23-channel-stay-night.sql
-- Validation example (RMB Aug 2026 vs Channel Production / Excel):
-- SELECT booking_source, SUM(room_revenue) AS revenue, SUM(rooms_sold) AS rooms_sold
--   FROM wp_lh_rpt_channel_stay_night
--  WHERE stay_date BETWEEN '2026-08-01' AND '2026-08-31'
--  GROUP BY booking_source ORDER BY booking_source;
-- Expect: Walk-In 516.00/14, Website 11791.19/278, Booking.com 14461.66/382, Hostelworld 18774.27/493
