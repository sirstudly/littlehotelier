...
export function createServer(): McpServer {
  ...
  server.tool(
    "get_booking",
    "Booking search (via ronbot-read-api). query may be a visible reservation id, third-party/OTA ref, or guest name. Name-like queries resolve via the local booking-assignment table (wp_lh_booking_assignment) first, then Cloudbeds get_reservation by id; Cloudbeds free-text search runs only when the local table has no matches. Guest names are only stored for current/future stays and those checked out within the last 2 weeks; older stays match by reservation id or OTA ref (a name matching only a recent stay can hide that guest's past stays — use search_reservations). Identifier-like queries go straight to Cloudbeds. Returns a list of matches (exact id matches preferred) — enough to pick a booking; for folio/notes/rooms call get_booking_timeline with reservationId. Prefer get_booking_timeline once when staff already have a unique ref. Capped at 20 matches with no date filter — to list every booking matching a name/term over a date range use search_reservations.",
    {
      property: propertySchema,
