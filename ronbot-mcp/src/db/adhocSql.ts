...
const BOOKING_ASSIGNMENT_NOTES =
  ...
  "guest_name, email, notes and comments are NULL on stays checked out more than 2 weeks ago (purged daily for privacy, on history rows too), so never filter on guest_name (e.g. LIKE '%tour%') for past stays: call search_reservations (query + date range) to get the reservation ids from Cloudbeds, then filter here with reservation_id IN (...). " +
